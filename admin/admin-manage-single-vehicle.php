  ...
  if(isset($_POST['upate_veh']))
    {
            ...
            $query="update tms_vehicle set v_location=?, v_pic=?, v_category=?, v_dpic=?, v_status=? where v_id = ?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('ssssssi', $v_location, $v_pic, $v_category, $v_dpic, $v_status, $v_id);
            $stmt->execute();
                if($stmt)
                {
                    $succ = "User Updated";
        ...
        ?>
          <form method ="POST" enctype="multipart/form-data"> 
            <div class="form-group">
                ...
                <input type="text" value="<?php echo $row->v_location;?>" required class="form-control" id="exampleInputEmail1" name="v_location">
            </div>
            <div class="form-group">Person In Charge</label>
                <input type="text" value="<?php echo $row->v_pic;?>" class="form-control" id="exampleInputEmail1" name="v_pic">
            </div>

            
            <div class="form-group">
              <label for="exampleFormControlSelect1">User Category</label>
            ...
            <hr>
            <button type="submit" name="upate_veh" class="btn btn-success">Update User</button>
          </form>
          <!-- End Form-->
          <?php }?>
        </div>
